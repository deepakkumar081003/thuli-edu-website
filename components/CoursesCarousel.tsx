...
export default function CoursesCarousel({ products }: CoursesCarouselProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        {products.map((p) => (
          <Link key={p.id} href={`/products/${p.slug}`} className="flex-none w-80 h-72">
            <div className="rounded-xl shadow-lg hover:shadow-2xl transition-transform hover:scale-105 cursor-pointer bg-white flex flex-col h-full">
              <img
                src={p.image_url}
                alt={p.title}
